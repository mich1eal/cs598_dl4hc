...
For the single multi-label model in our draft, we used the author's optimal hyperparameters with the exception of the loss function and learning rate. The original Keras models used categorical cross entropy, which has no direct pyTorch equivalent. In its place we CrossEntropyLoss. We also doubled the researchers' learning rate to 0.002.
