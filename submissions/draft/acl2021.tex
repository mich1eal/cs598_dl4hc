...
A single-direction Long Short Term Memory (LSTM) Recurrent Neural Network (RNN) was implemented with 100 parameters in its hidden state. The model also included an embedding layer (using GLoVE embeddings). A single fully connected layer was used to reduced the LSTM output to the 9 classes. A sigmoid activation function output per-class probabilities. 
