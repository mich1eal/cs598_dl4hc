...
We've chosen to test the paper's central hypothesis: that schemas can be automatically derived from patient thought records, and that this can be done with accuracy greater than random chance.

The paper used GLoVE embeddings to encode thought records. The researchers tested these with four types of models:
\begin{itemize}
    \item k-nearest neighbors (classifier + regression algorithms)
    \item Support vector machines (classifier + regression algorithms)
...
To quantify and compare model performance, the researchers used Spearman's rank-order correlation between predictions and ground truth. The researchers established ground truth by rating how well each thought record represented each schema on a scale of 0 to 3, where 0 indicated "no correspondence" and 3 indicated "complete correspondence".

The researchers found that their per-schema LSTMs outperformed the other models on all schemas except two: "power and control" and "meta-cognition". Those two schemas have low numbers of associated thought records.

The researchers also found that their regression models performed better with the "power and control" schema, while the classification models performed better with "health".
