...
\begin{itemize}
  ...
  \item Use a hierarchical model to take advantage structure in downward arrow data
...
\subsection{Computational Feasibility}
This study relies on a relatively small dataset (5747 utterances). The models used are not especially complicated. \citet{burger_2021} performed a grid search which took on the order of 40 hours to complete. If computational time becomes prohibitive, we plan to reduce the scope our grid while still confirming key results. 
